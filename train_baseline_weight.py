...
def main(args):
    ...
    elif args['dataset'] == 'ShanghaiA2B_ot_cluster':
        train_file = './npydata/A2B_cluster_ot.npy'
        test_file = './npydata/ShanghaiB_test.npy'
        weight_file = './npydata/A2B_cluster_ot_weight.npy'
    elif args['dataset'] == 'ShanghaiA2Q_ot_cluster':
        train_file = './npydata/A2Q_cluster_ot.npy'
        test_file = './npydata/qnrf_test.npy'
        weight_file = './npydata/A2Q_cluster_ot_weight.npy'
    ...
    with open(weight_file, 'rb') as outfile:
        weight_list = np.load(outfile).tolist()
    ...
    criterion = nn.MSELoss(reduce=False).cuda() #返回每个样本的loss
    ...
    if args['preload_data'] == True:
        train_data = pre_data(train_list,args, train=True,weight=weight_list)
...
# def selected_patches_by_OT():
#     #return train_list

def pre_data(train_list,args,train,weight=None):
    ...
    for j in range(len(train_list)):
        ...
        blob['fname'] = fname
        if train == True:
            blob['weight'] = weight[j]
        # print(blob)
        data_keys[count] = blob
        count += 1
    print("End Pre_load dataset ......")
    return data_keys
...
def train(Pre_data, model, criterion, optimizer, epoch, args):
    ...
    for i, (fname, img, fidt_map, kpoint, weight) in enumerate(train_loader):
        ...
        weight = weight.unsqueeze(1).view(weight.size(0),1,1,1).cuda()#(16*1)
        # print('loss_shape:',loss.shape)#(16*1*256*256)
        # print('weight_shape:',weight.shape)#(16*1)
        # print('loss_device',loss.device)
        # print('weight_device', weight.device)
        weighted_losses = weight * loss
        weighted_losses = torch.sum(weighted_losses)

        losses.update(weighted_losses.item(), img.size(0))
...
def validate(Pre_data, model, args):
    ...
    for i, (fname, img, fidt_map, kpoint) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            count, pred_kpoint, f_loc = LMDS_counting(d6, i + 1, f_loc, args)  # 将密度图转换为点图
...
def generate_point_map(kpoint, f_loc, rate=1):
    ...
    pred_coor = np.nonzero(kpoint)  # 取点图的非零元素
